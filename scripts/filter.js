//lo hice de esta forma porque asi lo habia iniciado y como lo habia avanzado lo continue así
...
const filterEvents = (category, searchText) => {
    return infoEventos.events.filter(event => {
...
const showEvents = events => {
    ...
    events.forEach(event => {
      ...
      <div class="card-body">
        ...
        <p>Price $ ${event.price}<a href="./details.html" class="btn btn-primary ms-4">See more</a></p>
      </div>
    </div>`;
    });
...
const updateEvents = () => {
    ...
    const filteredEvents = filterEvents(selectedCategories, searchText);
    showEvents(filteredEvents);
};

const categoriesHtml = [...new Set(infoEventos.events.map(event => event.category))].map(category => {
